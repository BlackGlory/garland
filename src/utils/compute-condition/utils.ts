...
import { asyncify } from 'extra-promise'
 ...
 * 
 * *由于V8没有实现尾调用优化, 在不使用asyncify包装的情况下使用该函数会有一定概率引起栈溢出.*
 */
const matchSequence = asyncify(
  async <Sequence extends ReadonlyArray<IToken<string> | INode<string>>>(
...
function isToken<T extends string = string>(val: unknown): val is IToken<T> {
  return isObject(val)
      && 'tokenType' in val && isString
      && 'value' in val && isString(val.value)
}

function isNode<T extends string = string>(val: unknown): val is INode<T> {
  return isObject(val)
